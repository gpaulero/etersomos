...
/**
 * GET /api/student/stream?key=cursos/xxx
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Only allow cursos/ prefix for this endpoint
    if (!key.startsWith('cursos/')) {
      return NextResponse.json({ error: 'Ruta no permitida' }, { status: 400 })
    }

    // 3. Verify enrollment — find the content item to get courseId
    const { db } = await import('@/lib/db')
    await ensureSchema()

    // Find the CourseContent item by r2Key
    ...
    // For video files, use inline disposition with streaming support
    const responseHeaders: Record<string, string> = {
      'Content-Type': contentType,
      'Content-Disposition': `inline; filename*=UTF-8''${encodeURIComponent(fileName)}`,
      'Cache-Control': 'no-store, no-cache, must-revalidate, proxy-revalidate',
      'X-Content-Type-Options': 'nosniff',
      // Prevent the file from being embedded in other sites
      'X-Frame-Options': 'SAMEORIGIN',
      // Content protection headers
      'X-Content-Protection': '1',
    }
    ...
    if (rangeHeader && result.ContentLength) {
      ...
      // We still return the full body; the browser will handle range slicing
      // For more precise range handling, we'd need to slice the stream
